...
router.post('/getworkoutbydate', authTokenHandler, async (req, res) => {
    const { date } = req.body;
    const userId = req.userId;
...
router.get('/getworkout', authTokenHandler, async (req, res) => {
    ...
    if (!user) {
        ...
        return res.json(createResponse(false, 'no user find'));
    }
